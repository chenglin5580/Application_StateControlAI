...
class ACNet(object):
    ...
    def _build_net(self, scope):  # 网络定义
        ...
        with tf.variable_scope('actor'):
            ...
            l_a = tf.layers.dense(self.s, 30, tf.nn.relu6, name='la')
            mu = tf.layers.dense(l_a, N_A, tf.nn.tanh,  name='mu')
            sigma = tf.layers.dense(l_a, N_A, tf.nn.softplus,  name='sigma')
        with tf.variable_scope('critic'):
            # l_c = tf.layers.dense(self.s, 100, tf.nn.relu6, kernel_initializer=w_init, name='lc')
            # v = tf.layers.dense(l_c, 1, kernel_initializer=w_init, name='v')  # state value
            l_c = tf.layers.dense(self.s, 100, tf.nn.relu6,  name='lc')
            v = tf.layers.dense(l_c, 1,  name='v')  # state value
...
while True:
    ...
    omega = GLOBAL_AC.choose_action(state_now)+10
